    ...
            </mt-header>
                <div style="position: relative;top:40px;left:16px;width:92%;"  @click="tabshow">
                    ...
                    <div style="position: relative;left:10px;margin-top:3px;color:#828282;font-size:15px;width:310px;">
                        <span>{{id==list.user_id?'我':'匿名网友唐僧哥'}}</span><span v-show="id==list.user_id"><van-tag  plain style="margin-left:10px">本人</van-tag></span><br/>
                        <span>{{$moment(list.time).format('YYYY-MM-DD')}}</span>
                    </div> 
                    <div id="treemain">
                        <span>{{list.treecontent}}</span><br/>
                            <img v-for="item in list.tree_pictures" :src="require(`../../../../koaS/koaPass/public/tree/${item.path}`)" style="width:105px;height:105px;margin-top:10px;margin-right:6px;">
                    </div>
                </div>
                <div style="width:395px;height:5px;background: rgb(231, 231, 231);position: relative;top:56px;"></div>
                <van-tabs v-model="active" color="#b0e6d1" line-width="30px" sticky :offset-top="30" style="position: relative;top:60px">
                    <van-tab title="回复">
                            <span  @click="tabshow" v-if="showalert" style="color:#828282;font-size:14px;letter-spacing: 2px;position:relative;left:142px;top:20px;">暂无回复</span>
                            <div class="contentout" v-for="(item,index) in commentList"  @click="tabshow">
                                    <img :src="require(`../../../../koaS/koaPass/public/images/${item.user.headpic}`)" width="32px" height="32px" 
                                    style="border-radius: 20px;">
                                    <span class="treename">{{item.user.user_name}}</span>
                                    <!-- <van-icon name="ellipsis" id="ellipsis" size="20px" @click="ellipsis"/> -->
                                    <br/>
                                    <span id="comment">{{item.content}}</span>
                                    <div class="likelike">
                                        <span style="color:#828282;font-size:13px;position: relative;left:23px;">{{$moment(item.time).format('MM-DD HH:mm')}}</span>
                                        <div class="rightlike">
                                            <!-- <van-icon name="comment-o" size="20px" style="margin-right:20px;"/>  -->
                                            <i class="iconfont icon-icon7" :style="userThumbs.includes(item.id)?color:''" ref="clickthumbs" @click="Tothumbs(index,item.id)"></i><span style="color:#666">{{item.thumbs==0?'赞':item.thumbs}}</span> 
                                        </div>
                                    </div>
                                </div>
                    ...
                    <van-tab title="点赞">
                            ...
                            <div class="thumbs">
                                <img src="../assets/pic/beauty.jpg" width="32px" height="32px" 
                                style="border-radius: 20px;">
                ...
                <mt-tab-item>
                    <van-button v-if="!showtab" size="small" @click="submit" style="position: relative;top:6px;left:6px;">发布</van-button>
                </mt-tab-item>
                <mt-tab-item>
                    <i v-if="showtab" class ="iconfont icon-fenxiang" style="font-size:30px;position:relative;left:-20px;"></i>
                </mt-tab-item>
                <mt-tab-item >
                    
                    <van-icon v-show="showtab" :name="goodname" @click="Thumbs" size="30px" style="position:relative;left:-10px;"/>
                </mt-tab-item>
        </mt-tabbar>
